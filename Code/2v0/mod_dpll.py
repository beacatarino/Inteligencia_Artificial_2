...
def Deduce(node, clause_list, decision):

	# node.clauses = factorize_clauses(node.clauses)

	conflict_list = []
	unit_dict = decision
	unit_dict, learned_from = find_unit_clause(node.clauses, clause_list)
	while unit_dict != {}:
...
def DecideNextBranch(node, symbols, level, backtrack, assign_list):
	...
	node = Node(deepcopy(node.clauses), new_model, level, node)
...
###############################################################################
# Main function: modified DPLL algorithm
...
def dpll(clauses, symbols):
	...
	# node initialization
	node = Node(clause_list, {}, 0, None)

	backtrack = False 	# indicates if there was a backtrack in the current 
	level = 0			#iteration

	while(True):

		level = level + 1
		node, decision = DecideNextBranch(node, symbols, level, backtrack, assign_list)
		backtrack = False

		while(True):
			level = level + 1
			node, status, conflict_list, conflict_var = Deduce(node, clause_list, decision)
			...
			if status == 'CONFLICT':
				...
				# while n != None:
					# if learned_clause != None:
						...
						# else:
							# id_clause = 0
						# node.clauses.append(Clause(id_clause, learned_clause))
						# n = n.parent
				if clause_list != []:
					id_clause = max([clause.id for clause in clause_list])+1
				else:
					id_clause = 0
				if learned_clause not in [c.term for c in clause_list]:
					clause_list.append(Clause(id_clause, learned_clause))
